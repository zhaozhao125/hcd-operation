        ...
        <div class="section-content" v-if="!section.slot">
          <el-form-item v-for="(item, index) in section.items" :key="index" v-show="!item.hidden" :label="item.label" :prop="item.name">
            ...
            <template v-else-if="item.type=='number'">
              ...
              <span class = "ml-5" v-if = "item.describe">{{item.describe}}</span>
            </template>
            <!-- 下拉选择 -->
            ...
            <template v-else-if="item.type=='switch'">
              <el-switch v-model="formModel[item.name]" :active-color = "item.activeColor" :inactive-color = "item.inactiveColor"
              :active-value = "item.activeValue" :inactive-value = "item.inactiveValue" @change="(item.events&&item.events.change)?$emit(item.events.change, formModel[item.name]):null">
              </el-switch>
            </template>
            <!-- 级联选择器 -->
            ...
            <template v-else-if="item.type=='radio'">
              <el-radio-group v-model="formModel[item.name]" @change="(item.events&&item.events.change)?$emit(item.events.change, formModel[item.name]):null">
                <el-radio v-for="(radioItem, radioIndex) in item.options" :key="radioIndex" :label="radioItem.value">{{radioItem.label}}</el-radio>
              </el-radio-group>
            </template>
            <!-- 多选 -->
            ...
            <template v-else-if="item.type=='textarea'">
              <el-input type="textarea" :autosize="{ minRows: 2, maxRows: 4}" :placeholder="item.placeholder || '请输入内容'" v-model="formModel[item.name]">
              </el-input>
            </template>
            <!-- 图片上传 -->
            <!--
            ...
            <template v-else-if="item.type=='file'">
              <el-upload class="upload-file" :action="upURL" :file-list="formModel[item.name]" :multiple="item.multiple" :data="getUploadParam(item.data)" :name="'file'" :limit="item.limit" :before-upload="beforeUploadCallbacks[item.name]" :on-success="uploadCallbacks[item.name]" :on-preview="handlePreviewFile" :on-remove="removeCallbacks[item.name]" :on-exceed="handelUploadExceed" :on-error="handleUploadError">
                <el-button size="small" type="primary" :disabled="isUploading || !upToken">点击上传</el-button>
          ...
          settingItem.items.forEach(item => {
            ...
            if (item.type == 'img' || item.type == 'file') {
              ...
              tmpBeforeUploadCallback[item.name] = file => {
                return this.handleBeforeUploadFile(item.name, file)
              }
            }
          })
        })
