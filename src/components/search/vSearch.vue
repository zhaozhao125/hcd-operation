        ...
        <el-col :md="8" :sm="12" :xs="24" v-for="(item, index) in searchSettings" :key="index" v-show="item.visible || showAll" v-if="item.hide?false:true">
          <el-form-item :label="item.label" :prop="item.name" style="max-width: 440px;">
            ...
            <template v-else-if="item.type=='select'">
              <el-select v-model="formModel[item.name]" :placeholder="item.placeholder" :multiple="item.multiple" :clearable="true" @change="(item.events&&item.events.change)?$emit(item.events.change, formModel[item.name]):null">
            ...
            <template v-else-if="item.type=='remoteStation'">
               <website-select v-model="formModel[item.name]" type="remoteStation" :placeholder="item.placeholder" @change="(item.events&&item.events.change)?$emit(item.events.change):null"></website-select>
             </template>
            <!--自定义label & 文本框-->
          ...
          if (!data[item.name] || data[item.name].some(item => !item.getTime)) {
            ...
            if (item.return) {
              ...
              if (item.return.format) {
                data[dateStart] = moment(date0).format(item.return.format)
                data[dateEnd] = moment(date1).format(item.return.format)
              } else {
