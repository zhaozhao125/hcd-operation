      ...
      <div class="x-data-detail" slot="content" v-if="detailData">
        ...
        <div class="x-detail-block">
          ...
          <el-row :gutter="10" type="flex">
            ...
            <el-col :xs="24" :sm="12" :md="8">
              <div class="x-detail-item">
                <div>
                  <el-button size="small" @click="handleShowCharge">充值</el-button>
                  <el-button size="small" @click="handleShowRefund">退款</el-button>
                </div>
              </div>
            </el-col>
          </el-row>
        ...
        <div class="x-detail-block">
          ...
          <el-row :gutter="10" type="flex">
            ...
            <el-col :xs="24" :sm="12" :md="8">
              <div class="x-detail-item">
                <div class="x-label">充电阈值</div>
                <div class="x-value">{{detailData.threshold}}元</div>
              </div>
            </el-col>
          </el-row>
        </div>
...
export default {
  ...
  methods: {
    ...
    getDetail(sn) {
      if (sn !== '') {
        this.$service.enterpriseDetails({ sn: sn }).then(res => {
          this.detailData = res.data.data
          this.isActive = res.data.data.opened
          this.formData = this.detailData
        })
        this.isActive = true
        this.detailData = { companyMoneyDTO: '1' }
      }
    },
    handleSwitch(active) {
      if (this.sn !== '') {
        console.log(this.sn)
        if (active) {
          this.$service.enterpriseClosed({ sn: this.sn }).then(res => {
            if (res.data.code == '0') {
          ...
          this.$service.enterpriseOpen({ sn: this.sn }).then(res => {
            if (res.data.code == '0') {
              this.isActive = !active
              this.$message({
                message: res.data.msg,
    ...
    handleCharge(data) {
      ...
      if (data.amount || data.amountPresentation) {
        this.$service.enterpriseCharge(data).then(res => {
          if (res.data.code == '0') {
            this.$message({
              message: '充值成功',
              type: 'success'
    ...
    handleRefund(data) {
      ...
      if (data.amount || data.amountPresentation) {
        this.$service.enterpriseRefund(data).then(res => {
          if (res.data.code == '0') {
            this.$message({
              message: '退款成功',
              type: 'success'
