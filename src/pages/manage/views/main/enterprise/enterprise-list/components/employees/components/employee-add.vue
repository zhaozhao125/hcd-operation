...
export default {
  ...
  methods: {
    ...
    queryAccount(num) {
      this.$service.employeesQueryAccount({ phone: num }).then(res => {
        console.log(res)
        if (res.data.code === '0') {
          this.hidden = true
          ...
          if (status === 'ok') {
            this.dec = '该手机号码已注册和充电，请核实号码准确性，确定后该号码将进入本企业，可享员工权益'
            ...
            this.dec = '该手机号码尚未注册和充电，请确认注册后，再进行添加'
            this.roleHidden = false
            this.addVisible = false
          }
        }
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (this.userId) {
            ...
            console.log(params)
            this.$service.createEmployees(params).then(res => {
              if (res.data.code === '0') {
