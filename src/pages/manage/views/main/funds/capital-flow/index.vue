...
export default {
  ...
  methods: {
    subjectList () {
      this.$service.subjectList({ userMoneyType: 'balance' }).then((res) => {
        this.searchSettings[2].options = res.data.data.map((ele) => {
          return {
            label: ele.userMoneyTypeText,
            value: ele.userMoneyType
          }
        })
      })
    },
      ...
      if (this.tableData.length < 1) {
        ...
        if (this.startCreatedOn) {
          if (((this.endCreatedOn - this.startCreatedOn) / 60 / 60 / 24) > 31) {
            ...
            this.$service.capitalFlowExport(
              params,
              this.$store.getters.token,
              '用户钱包流水列表.xlsx',
              api.CAPITAL_FLOW_EXPORT)
          }
        } else {
    ...
    loadTableData() {
      this.$service.capitalFlow(this.searchData, this.page).then(res => {
