...
export default {
  ...
  methods: {
    ...
    treeRender,
    getAllAuthURL(param) {
      let ret = param.map(item => item.id);
      [].forEach.call(param, item => {
        if (item.children && item.children.length) {
          ret = ret.concat(this.getAllAuthURL(item.children))
        }
      })
      return ret
    },
    handleCheckAll() {
      console.log('====this.authData====', this.authData)
      console.log('========', this.getAllAuthURL(this.authData.authUrls))
      this.$refs.authTree.setCheckedKeys(this.getAllAuthURL(this.authData.authUrls))
    },
    handleClearAll() {
