  ...
  <div id="station-details">
    <v-page :visible.sync="visible" :before-close="handleBeforeClose">
      ...
      <div  class="detail-content" slot="content" v-if = "detailData">
        <detail-basic :detailSettings='detailSettings' :detailData="detailData" :formData="detailData" ref = "vDetail">
          <template slot-scope="slot" slot = "prices_txt">
             <el-row  v-for = "(ele, index) in detailData.prices" :key = 'index' class = "charge_fee_list" style = "margin-right: 260px">
              <el-col :span="20"><div>{{ele.startTime}} - {{ele.endTime}} 电费{{ele.elecPrice}}元/度，服务费{{ele.servicePrice}}元/度</div></el-col>
            </el-row>
          </template>
