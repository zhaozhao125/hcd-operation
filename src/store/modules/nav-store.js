...
let getSessionData = itemName => {
  ...
  let hcdPlatformType = sessionStorage.getItem('hcdPlatformType')
  if (hcdPlatformType != window.HCD_PLATFORM_TYPE) {
    return null
  }
...
const getters = {
  ...
  tabs: state => {
    sessionStorage.setItem('hcdPlatformType', window.HCD_PLATFORM_TYPE)
